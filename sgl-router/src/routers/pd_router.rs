...
impl PDRouter {
    ...
    pub async fn route_generate(
        ...
        // Extract headers upfront to avoid Send issues
...
impl RouterTrait for PDRouter {
    ...
    async fn route_generate(
        ...
        if let Ok(openai_req) = serde_json::from_value::<GenerateRequest>(body.clone()) {
            let pd_req = openai_req.to_pd_request();
            return PDRouter::route_generate(self, client, req, pd_req, "/generate").await;
        }

        // Try PD format
        match serde_json::from_value::<GenerateReqInput>(body) {
            Ok(pd_req) => PDRouter::route_generate(self, client, req, pd_req, "/generate").await,
            Err(e) => (
